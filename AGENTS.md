...
8. **Update 咗嘢要同步 `my_agent_setting` repo** — 一旦安裝／更新咗新 skill、改咗 config（`settings.yaml`、`cordis.patch.yml` 等）、更新咗 `AGENTS.md`、或者改咗 knowledge base，要**順手同步去 `Desktop/my_agent_setting/`**（即 `C:\Users\cheun\Desktop\my_agent_setting`，入面係 `~/.dsh/` + knowledge 嘅鏡像，方便搬機／還原／備份：有 `skills/`、`knowledge/`、`settings.yaml`、`AGENTS.md`）。同步完先當該 task 完成。教訓：2026-08-16 n8n skills — 裝咗 15 個 skills 但唔記得 sync 去 my_agent_setting。
