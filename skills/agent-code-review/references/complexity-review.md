...
Read this reference for CC/CRAP requests, complex interacting decisions, or weak coverage. Use metrics to locate review targets, then inspect their behavior. Contribute findings to the main review without restarting its workflow; keep complexity-only requests within their requested scope. Edit code only when requested.
...
The evidence requirements and review workflow above are this reference's adaptations. Consult the installed analyzer's documentation when its semantics are uncertain.
